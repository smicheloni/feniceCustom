...
//	}	//	afterDelete
	...
	/**
	 * Calculates the different weights of a product, based on its default BOM.
	 * It calculates Net Weight, Tare Weight and Total Weight.
	 * @param weightUOMID UOM to be calculated
	 */
		...
		if(isBOM()) {
			if(isVerified()) {  // verified BOM: calculate components weight
				...
				/*bomLines.stream().forEach( bomComponent -> {
					MProduct product = bomComponent.getProduct();	
					product.calculateUOMWeights(weightUOMID, bom.getBOMType().compareTo("PK")==0);
				});*/
				...
				for( MPPProductBOMLine bomLine : bomLines) {
					...
					if(isProductPackage)
						tareWeightProduct = tareWeightProduct.add(weight);
					else
						netWeightProduct = netWeightProduct.add(weight);
				};

				setTareWeight(tareWeightProduct);
				setNetWeight(netWeightProduct);
				BigDecimal totalWeightProduct = tareWeightProduct.add(netWeightProduct);
				setWeight(totalWeightProduct);
				setCalculatedUOM(weightUOMID);
				saveEx();
